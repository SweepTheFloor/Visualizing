...
def change_perspective(img):
 ...
 ` # offset = img_size[0]*.25
  ...
  # return warped
    ...
    height, width = img.shape[0], img.shape[1]
    print height, width
    roi = img[np.int32(4.0/7.0*height):np.int32(6.0/7.0*height), 0:width]
...
def lr_curvature(binary_warped):
  ...
  histogram = np.sum(binary_warped, axis=0)#(binary_warped[600:,:], axis=0)
...
def process_image(img):
  ...
  plt.show()
  print("warp dimension, ", warped_image.shape[0],"  ",warped_image.shape[1])
